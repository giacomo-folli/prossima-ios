...
export default function SessionScreen() {
	...
	const handleComplete = useCallback(async () => {
		...
		if (result.celebrated) {
			setCelebData({ dur, label });
			setCelebrated(true);
		}
	}, [activeSession, completeSession]);

	const handleCancel = () =>
		Alert.alert("Cancel Session", "Progress will be lost.", [
			...
			<>
				...
				>
					<View style={styles.greetBlock}>
						<Text
							style={[styles.readyLabel, { color: colors.mutedForeground }]}
						>
							NEXT UP
						</Text>
						<Text style={[styles.readyDay, { color: colors.foreground }]}>
							{today?.label}
						</Text>
						<Text style={[styles.readyPlan, { color: colors.mutedForeground }]}>
							{plan.name}
						</Text>
					</View>
				...
				<View style={[styles.bottomBar, { bottom: botPad, paddingBottom: 16 }]}>
					...
					>
						<Text
							style={[styles.pillBtnText, { color: colors.primaryForeground }]}
						>
							Begin {today?.label}
						</Text>
					</Pressable>
	...
	const totalSets = activeSession.dayExercises.reduce(
		(a, ex) => a + ex.sets,
		0,
	);
		...
		<LinearGradient colors={gradientColors} style={styles.root}>
			...
			>
				<View style={styles.sessionMeta}>
					<Text
						style={[styles.sessionDayLabel, { color: colors.mutedForeground }]}
					>
			...
			<View style={[styles.overallTrack, { backgroundColor: colors.border }]}>
				<View
					style={[
						styles.overallFill,
						{ backgroundColor: colors.accent, width: `${overallProg * 100}%` },
					]}
				/>
			</View>
			...
			<ScrollView
				...
				contentContainerStyle={[
					styles.scrollContent,
					{ paddingTop: 16, paddingBottom: botPad + 80 },
				]}
				showsVerticalScrollIndicator={false}
				contentInsetAdjustmentBehavior="never"
			>
			...
			{logState && (
				<SetLogModal
					...
					previousWeight={
						getEntries(logState.exercise.name).slice(-1)[0]?.weightKg
					}
