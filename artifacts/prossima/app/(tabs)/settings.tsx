...
const SettingSection: React.FC<SettingSectionProps> = ({ label, children, footer }) => {
  const colors = useColors();
  const { resolvedScheme } = useTheme();
  return (
    <View style={styles.sectionContainer}>
      {label && <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>{label}</Text>}
      <GlassView
      ...
      {footer && <Text style={[styles.sectionFooter, { color: colors.mutedForeground }]}>{footer}</Text>}
    </View>
  );
};
...
const SettingRow: React.FC<SettingRowProps> = ({
  ...
  const rowContent = (
    <View style={[styles.rowContainer, !isLast && { borderBottomColor: colors.separator, borderBottomWidth: StyleSheet.hairlineWidth }]}>
      <View style={[styles.iconContainer, { backgroundColor: iconBg }]}>
        <Ionicons name={icon} size={15} color="#FFFFFF" />
      </View>
      <View style={styles.rowTextContainer}>
        <Text style={[styles.rowLabel, destructive ? { color: colors.destructive } : { color: colors.foreground }]} numberOfLines={1}>
          {label}
        </Text>
        {sublabel && (
          <Text style={[styles.rowSublabel, { color: colors.mutedForeground }]} numberOfLines={2}>
            {sublabel}
          </Text>
        )}
      </View>
...
export default function SettingsScreen() {
  ...
  const handleReset = () =>
    Alert.alert('Reset All Data', `Delete all ${sessions.length} sessions permanently? This cannot be undone.`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Reset', style: 'destructive', onPress: resetAllData },
    ]);

  const handleResetYaml = () =>
    Alert.alert('Reset Plan', 'Replace current plan with default Push Pull Legs?', [
      { text: 'Cancel', style: 'cancel' },
  ...
  return (
    <LinearGradient colors={gradientColors} style={{ flex: 1 }}>
      ...
      >
        ...
        {ok && (
          <View style={[styles.toastBanner, { backgroundColor: colors.success + '20', borderColor: colors.success }]}>
            <Ionicons name="checkmark-circle" size={15} color={colors.success} />
            <Text style={[styles.toastText, { color: colors.successForeground || colors.foreground }]}>Plan updated successfully</Text>
          </View>
        )}
        ...
        <SettingSection label="APPEARANCE">
          <SettingRow
            ...
            isLast={true}
            rightContent={
              <View style={[styles.segmentedContainer, { backgroundColor: colors.secondary }]}>
        ...
        <SettingSection label="TRAINING PLAN">
          <SettingRow
            ...
            sublabel={plan ? `${plan.days.length} days · ${plan.days.reduce((a, d) => a + d.exercises.length, 0)} exercises` : 'Configure a plan to start training'}
            isLast={false}
          />
          <SettingRow
            icon="code-slash"
        ...
        >
          <LinearGradient colors={gradientColors} style={styles.modalRoot}>
            <View style={[styles.modalHeader, { borderBottomColor: colors.separator }]}>
              ...
              <Text style={[styles.modalTitle, { color: colors.foreground }]}>Edit YAML Plan</Text>
              <Pressable
                onPress={handleSave}
                disabled={saving}
                accessibilityRole="button"
                ...
                style={styles.modalHeaderBtn}
              >
                <Text style={[styles.modalHeaderBtnText, { color: colors.primary, fontWeight: '600' }]}>
            ...
            >
              ...
              >
                <Text style={[styles.modalHelpText, { color: colors.mutedForeground }]}>
                  Define your days, exercises, sets, reps, rest timers, muscles, and notes in YAML format.
                </Text>

                {parseError && (
                  <View style={[styles.modalErrorBanner, { backgroundColor: colors.destructive + '14', borderColor: colors.destructive }]}>
                    <Ionicons name="warning-outline" size={16} color={colors.destructive} />
                    <Text style={[styles.modalErrorText, { color: colors.destructive }]}>{parseError}</Text>
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  screenTitle: { fontSize: 36, fontWeight: '300', fontFamily: 'Inter_400Regular', letterSpacing: -0.5, marginBottom: 4 },
